...
package org.eclipse.equinox.http.jetty.internal.Overriden;
...
public class MyHttpServerManager extends HttpServerManager {
	...
	public MyHttpServerManager(File workDir) {
		super(workDir);
	...
	public static class InternalHttpServiceServlet implements HttpSessionListener, HttpSessionIdListener, Servlet {
		...
		public InternalHttpServiceServlet() {
			...
			/*
			 * try { sessionDestroyed = clazz.getMethod("sessionDestroyed", new Class<?>[]
			 * {String.class}); //$NON-NLS-1$ } catch (Exception e) { throw new
			 * IllegalStateException(e); } try { sessionIdChanged =
			 * clazz.getMethod("sessionIdChanged", new Class<?>[] {String.class});
			 * //$NON-NLS-1$ } catch (Exception e) { throw new IllegalStateException(e); }
			 */
		}
